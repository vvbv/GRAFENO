...
Every task follows a pipeline with five configurable roles (CLI + model for each, and optionally a per-model effort level when the CLI exposes one — e.g. `opencode run ... --variant <nivel>`). The model selector in the configuration screen and in the per-task agents modal supports live text filtering by substring, which helps when a CLI exposes a long catalogue.

0. **First step** *(optional)* — runs only when a `first_prompt` is defined, either globally (`first_prompt` in `~/.grafeno/config.toml`) or for the specific task (`First step instructions` field in the new-task form; it inherits the global value and can be emptied per task to disable the step). It carries its own CLI + model + effort (`cfg.role.first`, the first entry of the per-task agents modal) and writes its report under `first/01-first.md`, exposed by the new **First** tab in the task detail. If no prompt is configured, the phase is skipped and the pipeline starts straight at planning.
...
**Trigger tasks**: a trigger is a task template that is spawned automatically at a pipeline phase boundary (`before` or `after`) of another task. There are two levels — global (`~/.grafeno/triggers.toml`, edited from the configuration screen `c`) and project (the `[[triggers]]` array of `<workdir>/.grafeno.toml`). Each trigger declares the phases it listens to (`all` or any subset of first step, plan, implement, review, fix, final steps, tests) and whether it fires before or after them. When it fires, a new independent GRAFENO task is created in automode and scheduled for the current minute, so the scheduler tick of the App starts it unattended: triggers never block nor interfere with the task that fired them, and tasks spawned by a trigger do not fire further triggers (no recursion).

**Processing profiles**: a profile is a named set of `CLI + model + effort` assignments for the five pipeline roles (first step, planner, implementer, reviewer, final steps). Profiles live in `~/.grafeno/profiles.toml` and are edited from the configuration screen (`c`, "Processing profiles" section — add / edit / delete with the same modal used for the per-task agents). At task creation time the new-task form shows a "Processing profile" selector (only when at least one profile exists): choosing one snapshots its roles into the task, overriding the global configuration just for that task; the detail screen displays the chosen profile in the agents bar. The Telegram bot asks the same question inline (right after the proposal confirmation and before chaining) when profiles are defined, and the REST API accepts an optional `profile` field in the create-task body (400 when the name is unknown). Picking a profile in the per-task agents modal (`e` on the detail screen) refills the form; if you then edit a role manually, the profile is cleared because the form no longer matches it, so the snapshot in `task.toml` stays consistent with what you actually run.
...
**Usage-limit retries**: when an agent CLI reports an exhausted quota/rate limit (`429`, `rate limit`, `quota exceeded`, `usage limit`, `insufficient_quota`, `out of credits`, `Token Plan usage limit reached`, etc.), GRAFENO does not fail the phase. If the message carries a `retry after` / `try again in` time hint, the orchestrator waits exactly that long and retries the same phase, reusing the session when possible. When there is no time hint, it probes every 60 s. After 30 quick attempts the phase switches to a passive wait mode: it keeps retrying every 15 minutes, indefinitely, until the quota resets — the task never fails because of usage exhaustion. While a phase is waiting, the tasks list and the `PhaseBar` append a `Waiting` suffix to the current state so you can tell at a glance that the pipeline is paused on quota, not stalled.

**Media**: the new-task form and the "Ask for more" modal accept pasted PNG images with `Ctrl+V` (or `Cmd+V` on macOS) on every platform that exposes a clipboard tool (`wl-paste`, `xclip`, `pngpaste` or `osascript`). The image is saved under the task's `media/` directory as `media-NN.png`, and a `media/media-NN.png` token is inserted at the cursor in place of the raw bytes. Images pasted before the task exists are buffered in the form and flushed once the task is created. Photos and videos that the user attaches to a Telegram message are saved to the `media/` directory of the first task created from that message (image suffixes become `media-NN.<ext>` tokens; videos and other files become absolute paths in the description), and the REST/WebSocket API accepts the same kind of attachments on `POST /api/v1/tasks` (and `tasks.create` over WS) through an optional `attachments` array of `{name, data-base64}` objects (max 10 per request); both channels go through the same `media.attach_files` helper. Audio attachments (ogg/mp3/wav/m4a/opus by file name) are additionally transcribed with the configured STT provider and the transcription is appended to the task description (the response also returns a `transcriptions` array listing each file with either its `text` or `error`). The detail view exposes a **Media** tab listing every saved image with its absolute path; clicking an entry shows the path and, when the terminal supports it (kitty / WezTerm / iTerm + the optional `textual-image` package), an inline preview, otherwise it opens the image with the system viewer (`xdg-open` / `open`). Vision-capable CLIs also receive the absolute paths of all attached images in the plan, re-evaluation and implementation prompts (review, fix and final-step prompts stay clean).
...
**Internal prompts language**: the instructions GRAFENO sends to the agent CLIs (first step, plan, re-evaluation, implementation, review, fixes, final steps, AGENTS.md generation and the Telegram bot parser/questions) are available in English and Spanish. By default they use the interface language; the configuration screen (`c`, Language section) can pick a separate one, e.g. an English GUI with Spanish prompts. It is persisted as `prompt_language` in `config.toml` (empty = same as the interface) and applies to the next agent run without restarting.
...
GET  /api/v1/tasks/{id}/artifacts?kind=&cycle=#  # first|plan|review|final .md files
POST /api/v1/tasks                             # create a task (body: name, workdir, ..., profile?: "<name>", attachments?: [{name, data-base64}] x10)
POST /api/v1/tasks/{id}/start|resume|restart|pause|discard|mark-done
POST /api/v1/tasks/{id}/extend                 # body: {"request": "..."}
POST /api/v1/audio/speech                     # TTS: {"text", format?} -> audio file (wav/ogg)
...
version, which lives in a single source of truth: `src/grafeno/__init__.py`
(`__version__`); `pyproject.toml` derives it dynamically at build time
(`tool.setuptools.dynamic`), so the package is always built with the
correct version. The workflow `.github/workflows/release.yml` detects the
bump, builds the package, verifies the built artifacts carry exactly that
version and publishes the GitHub Release with the `vX.Y.Z` tag and
attached artifacts. Once a new release is out, existing installs pick it
up via the in-app self-update check (or on demand with `grafeno update`)
— see the *Self-update* note in [How it works](#how-it-works).
...
| `c` | Detail | Continue a FAILED or interrupted task from the phase where the pipeline stopped, reusing every artifact already on disk (with confirmation) |
| `R` | Detail | Restart the task from scratch (aborts the current run if any, then resets to `DRAFT` and clears `first/`, `plan/`, `review/`, `final/` and the schedule, with confirmation) |
...
```
~/.grafeno/
├── config.toml              # language (en/es), prompt_language (internal prompts; empty = same as language), roles (cli+model+effort, including the optional first step), first-step prompt, automode, tests, git, theme (palette), final-steps prompt, global hook, editor, auto_update (agent CLIs), self_update (GRAFENO itself), workspaces (root folders for project discovery), api (enabled/host/port/tokens for the REST + WebSocket server)
