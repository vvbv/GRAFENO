...
class Orchestrator:
    ...
    def _plan_files(self) -> list[Path]:
        plan_dir = paths.plan_dir(self.task.id)
    ...
    async def _execute(
        ...
        result = await driver.run(
            request, on_event=lambda event: self._on_event(phase, event)
        )
    ...
    async def run_plan(self) -> None:
        ...
        if not self._plan_files():
            ...
            plan_path = paths.plan_dir(self.task.id) / "01-plan.md"
    ...
    async def run_review(self) -> Verdict:
        ...
        review_path = paths.review_dir(self.task.id) / f"{review_number:02d}-review.md"
    ...
    async def run_tests(self) -> bool:
        ...
        self._info(f"[Tests] Código de salida: {returncode}")
        return returncode == 0
